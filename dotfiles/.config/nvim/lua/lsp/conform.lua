...
-- oxfmt is the markdown formatter (replaces mdformat). It preserves Obsidian
-- wikilinks/embeds/callouts and YAML frontmatter that mdformat mangled
-- (wikilink escaping, 70-underscore thematic breaks, `\` hard breaks).
-- CHANGELOG.md is generated by semantic-release with `*` bullets; oxfmt (like
-- mdformat) normalizes to `-`, so exclude it here. A `condition` gates every
-- call site (autosave AND the `=G` mapping), unlike format_on_save's return.
conform.formatters.oxfmt = {
  condition = function(_, ctx)
    return vim.fs.basename(ctx.filename or ""):lower() ~= "changelog.md"
  end,
}
